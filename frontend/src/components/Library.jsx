...
const Library = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        ...
        console.error(
          "Failed to load library (Ensure backend is running)",
          error,
        );
      }
    };
  ...
  const deleteLog = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to magically poof this book away? 🗑️",
      )
    )
      return;
    try {
  ...
  return (
    <div className="animate-pop">
      ...
      >
        ...
        <button className="btn btn-accent" onClick={shareList}>
          Share My List 📤
        </button>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="card">
          <p style={{ fontSize: "1.2rem", fontWeight: "bold" }}>
            Your library is empty. Go discover some amazing books!
          </p>
        </div>
      ) : (
        <div
